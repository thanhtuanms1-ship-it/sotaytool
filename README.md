...
<body>
    ...
    <script>
        ...
        // ĐỌC FILE TỪ BỘ NHỚ ĐIỆN THOẠI VÀ GHI ĐÈ FILE MỚI NHẤT
        function importTxtFile(el) {
            ...
            const r = new FileReader(); r.onload = function(e) {
                ...
                if (tmp.length === 0) alert("Cấu trúc file không đúng! Hàng đầu phải chứa cột ngăn cách bằng Tab hoặc dấu phẩy.");
                else { dbKnives = tmp; localStorage.setItem('dbKnives', JSON.stringify(dbKnives)); updateStatus(); alert(`Đã làm sạch bộ nhớ cũ & nạp mới thành công ${dbKnives.length} dao!`); searchKnife(); }
                el.value = '';
            }; r.readAsText(el.files[0], 'UTF-8');
        }

        // TRA CỨU ĐỘ LỆCH TOÁN HỌC ±0.05
        ...
        // THÔNG THUẬT QUẢN LÝ LÝ LỊCH CHẾ DAO
        ...
        function renderHistory() {
            ...
            historyList.forEach(i => { tb.innerHTML += `<tr><td>${fmtDate(i.date)}</td><td>${i.original}</td><td>${i.replace}</td><td><span style="font-weight:bold;color:var(--p)">${i.type}</span></td><td><button class="btn btn-d" onclick="delItem(${i.id})">Xóa</button></td></tr>`; });
        }
        ...
        // BẢN MẠCH SAO LƯU/KHÔI PHỤC TOÀN CỤC (.JSON)
